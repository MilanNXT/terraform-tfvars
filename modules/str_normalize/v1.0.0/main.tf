# normalization (https://github.com/Olivr/terraform-null-normalize/tree/main)
...
variable "length" {
    description = "maximal string length to return."
    type        = number
    default     = 20
}
...
locals {
  ...
  normalized1   = replace(local.latinized, "/[^a-zA-Z0-9-\\s-]/", "")
  normalized2   = trimspace(replace(local.normalized1, "/\\s+/", " "))
  normalized3   = replace(local.normalized2, "/\\s/", "-")
  normalized4   = replace(local.normalized3, "/-{2,}/", "-")
  normalized    = trim(substr(local.normalized4,0,var.length),"-_")
}
